...
echo "Kernel version $kernel_version (from '$UNAME_R'; '$UNAME_V') on $UNAME_R"
...
if [ $major_version -eq 5 -a $minor_version -lt 13 ]; then
	...
	pushd $hda_dir; patch -b -p2 <../../patch_patch_cirrus.c.diff
	popd
	...
	if [ $isubuntu -ge 1 ]; then
		...
		pushd $hda_dir; patch -b -p2 <../../patch_patch_cs8409.c.diff
		popd

		if [ $iscurrent -ge 0 ]; then
			pushd $hda_dir; patch -b -p2 <../../patch_patch_cs8409.h.diff
			popd
		else
			pushd $hda_dir; patch -b -p2 <../../patches/patch_patch_cs8409.h.ubuntu.pre51547.diff
			popd
		fi
		...
		if [ $iscurrent -ge 0 ]; then
			pushd $hda_dir; patch -b -p2 <../../patch_patch_cirrus_apple.h.diff
			popd
		fi
		...
		if [ $iscurrent -ge 0 ]; then
			...
			pushd $hda_dir; patch -b -p2 <../../patches/patch_patch_cs8409.h.main.pre519.diff
			popd
		fi
